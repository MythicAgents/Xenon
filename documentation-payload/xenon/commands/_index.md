...
| `jump_psexec`  | `jump_psexec -Target <host> -Payload <file> [-Command <extra args>] [-ServiceName <name>] [-Username <user>] [-Domain <domain>] [-Password <pass> \| -Hash <ntlm>]` | Lateral movement via Windows Service Control Manager. Uploads a payload to `ADMIN$\Temp` under a random 10-character name, creates a temporary service to execute it, then deletes the service. Supports alternate credentials (plaintext or pass-the-hash). Requires admin rights on the target. |
| `wmiexecute`   | `wmiexecute -command <cmd> [-host <host>] [-username <user>] [-password <pass>] [-domain <domain>]` | Script-only WMI wrapper. Spawns a process on a local or remote host via `wmic` `Win32_Process::Create`. Optionally supply alternate credentials. |
| `jump_wmi`     | `jump_wmi -Target <host> -Payload <file> [-Command <extra args>] [-Username DOMAIN\user] [-Password pass]` | Lateral movement via WMI `Win32_Process::Create`. Uploads a payload to `ADMIN$\Temp` under a random 10-character name and spawns it via WMI. If WMI fails, automatically falls back to sc (psexec-style) service execution. Optionally supply explicit credentials; otherwise the current token is used. |
| `dcomexec`     | `dcomexec -command <cmd> [-host <host>] [-method mmc20\|shellwindows]` | Script-only DCOM execution wrapper. Spawns a process on a local or remote host via `MMC20.Application` or `ShellWindows` using PowerShell. |
| `jump_dcom`    | `jump_dcom -Target <host> -Payload <file> [-Method mmc20\|shellwindows] [-Command <extra args>]` | Lateral movement via DCOM (`MMC20.Application` or `ShellWindows`). Uploads a payload to `ADMIN$\Temp` under a random 10-character name and spawns it via DCOM. If DCOM fails, automatically falls back to sc (psexec-style) service execution. |
